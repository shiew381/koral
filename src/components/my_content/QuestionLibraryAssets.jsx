...
async function fetchQueriedImages(searchTerm, libraryID, setImages) {
  const ref = firebase
    .firestore()
...
async function deleteLibraryImage(libraryID, fileName, docID, setImageIndex) {
  const imageStorageRef = firebase
    .storage()
    .ref()
    .child(`libraries/${libraryID}/images/${fileName}`);
...
export default function QuestionLibraryAssets({ libraryID, libraryTitle }) {
      ...
      >
        <Fade in={open}>
          <Box className="modal-form-v2 modal-common-styling">
            <Typography variant="h5">{libraryTitle}</Typography>
                  ...
                  <Box padding={1} minHeight={30} minWidth={250}>
                    ...
                    {file ? (
                      <FileUpload
                        ...
                        file={file}
                        setFile={setFile}
                        storagePath={`libraries/${libraryID}/images/${file.name}`}
