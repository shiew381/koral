...
export default function ImportFromLibrary({
  ...
  const allLibrariesInfo = [myLibraryInfo, ...librariesInfo];
  ...
  const communityLibraryInfoRef = firebase
    .firestore()
  ...
  const communityLibraryRef = communityLibraryInfoRef.collection("questions");

  const libraryInfoRef =
    libraryIndex === 0 ? myLibraryInfoRef : communityLibraryInfoRef;
  const libraryRef = libraryIndex === 0 ? myLibraryRef : communityLibraryRef;
  ...
  function fetchLibrariesInfo() {
    firebase
      ...
      .onSnapshot((snapshot) => {
        ...
        setLibrariesInfo((prevState) => fetchedItems);
      });
  }
